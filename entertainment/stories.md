...
- Spring & Autumn Brushwork: Plant clues deep and many and scattered (using details to drive hidden plot lines). Uses characters' hidden motivations to connect to different parts, and deceive readers' attention by focusing on parts he wants you to focus on, making re-reading possible. (e.g. don't get fooled by (maybe unreliable) narrator's viewpoint, some characters are behave differently from how they are written, use similar but subtly different events to reveal details, some characters know too much (you need to use characters view not god), etc). His stories are very consistent (to the details), and sometimes use inconsistencies to reveal lies.
