...
>    Since there must be other factors at play,it is tied to non-local state.
...
5. Prefer relative imports, but not outside the current module [^4].
6. Use autoflake, isort, black to format the code, default settings.
...
[^4]: Example: `from .pkg import Module` is OK, but not `from ..pkg import Module`, directly use `from our.pkg import Module` if needed (assuming `our` is current package).
